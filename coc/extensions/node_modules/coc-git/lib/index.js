...
var require_america = __commonJS((exports2, module2) => {
  module2["exports"] = function(colors4) {
    return function(letter, i, exploded) {
      if (letter === " ")
        return letter;
      switch (i % 3) {
        case 0:
          return colors4.red(letter);
        case 1:
          return colors4.white(letter);
        case 2:
          return colors4.blue(letter);
      }
    };
  };
});
...
var require_zebra = __commonJS((exports2, module2) => {
  module2["exports"] = function(colors4) {
    return function(letter, i, exploded) {
      return i % 2 === 0 ? letter : colors4.inverse(letter);
    };
  };
});
  ...
  module2["exports"] = function(colors4) {
    ...
    return function(letter, i, exploded) {
      if (letter === " ") {
        ...
        return colors4[rainbowColors[i++ % rainbowColors.length]](letter);
      }
    };
  };
});
...
var require_random = __commonJS((exports2, module2) => {
  module2["exports"] = function(colors4) {
    ...
    return function(letter, i, exploded) {
      return letter === " " ? letter : colors4[available[Math.round(Math.random() * (available.length - 2))]](letter);
    };
  };
});
...
var require_colors = __commonJS((exports2, module2) => {
  ...
  var ansiStyles = colors4.styles = require_styles();
  var defineProps = Object.defineProperties;
  var newLineRegex = new RegExp(/[\r\n]+/g);
  colors4.supportsColor = require_supports_colors().supportsColor;
  if (typeof colors4.enabled === "undefined") {
    colors4.enabled = colors4.supportsColor() !== false;
  }
  colors4.enable = function() {
    colors4.enabled = true;
  };
  colors4.disable = function() {
    colors4.enabled = false;
  };
  colors4.stripColors = colors4.strip = function(str) {
    return ("" + str).replace(/\x1B\[\d+m/g, "");
  };
  var stylize = colors4.stylize = function stylize2(str, style) {
    if (!colors4.enabled) {
      return str + "";
    }
    var styleMap = ansiStyles[style];
    if (!styleMap && style in colors4) {
      return colors4[style](str);
    }
  ...
  var proto = defineProps(function colors5() {
  }, styles);
  function applyStyle() {
  ...
  colors4.setTheme = function(theme) {
    ...
    for (var style in theme) {
      (function(style2) {
        colors4[style2] = function(str) {
          if (typeof theme[style2] === "object") {
            var out = str;
            for (var i in theme[style2]) {
              out = colors4[theme[style2][i]](out);
            }
            return out;
          }
          return colors4[theme[style2]](str);
        };
      })(style);
    }
  };
  ...
  colors4.trap = require_trap();
  colors4.zalgo = require_zalgo();
  colors4.maps = {};
  colors4.maps.america = require_america()(colors4);
  colors4.maps.zebra = require_zebra()(colors4);
  colors4.maps.rainbow = require_rainbow()(colors4);
  colors4.maps.random = require_random()(colors4);
  for (var map in colors4.maps) {
    (function(map2) {
      colors4[map2] = function(str) {
        return sequencer(colors4.maps[map2], str);
      };
    })(map);
  }
  defineProps(colors4, init());
});

// node_modules/colors/safe.js
var require_safe = __commonJS((exports2, module2) => {
  var colors4 = require_colors();
  module2["exports"] = colors4;
});
...
function findGitWin32InPath(onLookup) {
  const whichPromise = new Promise((c, e) => import_which.default("git.exe", (err, path8) => err ? e(err) : c(path8)));
...
function findSpecificGit(path8, onLookup) {
  return new Promise((c, e) => {
    onLookup(path8);
    const buffers = [];
    const child = import_child_process.spawn(path8, ["--version"]);
  ...
  constructor(nvim, manager) {
    ...
    this.addMultipleAction("commit", async (items) => {
      ...
      try {
        ...
        import_coc6.window.showMessage(`Gcommit command failed, make sure fugitive installed.`, "error");
      }
    });
    this.addAction("reset", async (item) => {
...
var GitStatus = class {
  constructor(service) {
    ...
    this.disposables = [];
    this._enabled = false;
    this.mutex = new import_coc7.Mutex();
    let config = import_coc7.workspace.getConfiguration("git");
    ...
    import_coc7.events.on("BufEnter", this.refresh, this, this.disposables);
    import_coc7.events.on("FocusGained", this.refresh, this, this.disposables);
    let timer;
    import_coc7.events.on("BufWritePost", () => {
      timer = setTimeout(() => {
        this.refresh();
      }, 300);
  ...
  dispose() {
    import_coc7.disposeAll(this.disposables);
  }
};
...
var DocumentManager = class {
  constructor(nvim, service, virtualTextSrcId, conflictSrcId = 0) {
    ...
    import_coc8.workspace.onDidCloseTextDocument((e) => {
      let buf = this.buffers.get(e.bufnr);
      if (buf)
        buf.dispose();
    ...
    import_coc8.events.on("CursorMoved", import_debounce.default(async (bufnr, cursor) => {
      let buf = this.buffers.get(bufnr);
      if (buf)
        await buf.showBlameInfo(cursor[0]);
    }, 100), null, this.disposables);
    import_coc8.events.on("BufWritePre", (bufnr) => {
    ...
    import_coc8.events.on("FocusGained", async () => {
  ...
  loadConfiguration(e) {
    ...
    let obj = {
      ...
      addGBlameToVirtualText: import_coc8.workspace.nvim.hasFunction("nvim_buf_set_virtual_text") && config.get("addGBlameToVirtualText", false),
  ...
  async push(args) {
    let bufnr = await import_coc8.workspace.nvim.call("bufnr", "%");
    let root = await this.resolveGitRootFromBufferOrCwd(bufnr);
    let extra = this.config.pushArguments;
    if (!root) {
      import_coc8.window.showMessage(`not belongs to git repository.`, "warning");
      return;
    }
    if (args && args.length) {
      await import_coc8.window.runTerminalCommand(`git push ${[...args, ...extra].join(" ")}`, root, true);
      return;
    }
    ...
    if (!remote) {
      import_coc8.window.showMessage(`remote not found`, "warning");
      return;
    }
    output = await repo.safeRun(["rev-parse", "--abbrev-ref", "HEAD"]);
    if (!output) {
      import_coc8.window.showMessage(`current branch not found`, "warning");
      return;
    }
    await import_coc8.window.runTerminalCommand(`git push ${remote} ${output}${extra.length ? " " + extra.join(" ") : ""}`, root, true);
  }
  get buffer() {
    return import_coc8.workspace.nvim.call("bufnr", "%").then((bufnr) => {
      let buf = this.buffers.get(bufnr);
      if (!buf)
        import_coc8.window.showMessage(`Cant't resolve git repository for current buffer.`, "warning");
      return buf;
    });
  }
  getBuffer(bufnr) {
...
var Git = class {
  ...
  async getRepositoryRoot(repositoryPath) {
    ...
    return import_path5.default.normalize(result.stdout.trim());
  }
...
async function exec2(child, cancellationToken) {
  ...
  const once = (ee, name, fn) => {
    ee.once(name, fn);
    disposables.push(import_coc9.Disposable.create(() => ee.removeListener(name, fn)));
  };
  const on = (ee, name, fn) => {
    ee.on(name, fn);
    disposables.push(import_coc9.Disposable.create(() => ee.removeListener(name, fn)));
  ...
  try {
    ...
    import_coc9.disposeAll(disposables);
  }
}
...
async function getRealPath(fullpath) {
  ...
  try {
    ...
    if (e.message.includes("ENOENT")) {
      try {
        resolved = await import_coc10.workspace.nvim.call("expand", [fullpath]);
      } catch (e2) {
      }
    }
  }
...
var Resolver = class {
  ...
  async resolveGitRoot(doc) {
    ...
    let fullpath = await getRealPath(import_coc10.Uri.parse(uri).fsPath);
    if (process.platform == "win32") {
      fullpath = import_path6.default.win32.normalize(fullpath);
    }
    if (!root) {
      let parts = fullpath.split(import_path6.default.sep);
      let idx = parts.indexOf(".git");
      if (idx !== -1) {
        root = parts.slice(0, idx).join(import_path6.default.sep);
        this.resolvedRoots.set(uri, root);
      } else {
        try {
          root = await this.git.getRepositoryRoot(import_path6.default.dirname(fullpath));
          if (import_path6.default.isAbsolute(root)) {
            this.resolvedRoots.set(uri, root);
          } else {
            root = void 0;
          }
    ...
    if (root) {
      this.relativePaths.set(uri, import_path6.default.relative(root, fullpath));
    }
    this.channel.appendLine(`resolved root of ${fullpath}: ${root}`);
    return root;
  }
  async resolveRootFromCwd() {
    let parts = import_coc10.workspace.cwd.split(import_path6.default.sep);
    let idx = parts.indexOf(".git");
    if (idx !== -1)
      return parts.slice(0, idx).join(import_path6.default.sep);
    try {
      return await this.git.getRepositoryRoot(import_coc10.workspace.cwd);
    } catch (e) {
    }
    return null;
  }
...
var GitBuffer = class {
  ...
  async chunkStage() {
    ...
    let diff = this.getChunk(line);
    if (!diff) {
      import_coc11.window.showMessage("Not positioned in git chunk", "error");
      return;
    }
    let head;
  ...
  async diffCached() {
    ...
    if (!res.trim()) {
      import_coc11.window.showMessage("Empty diff");
      return;
    }
    let {nvim} = import_coc11.workspace;
    nvim.pauseNotification();
  ...
  async nextConflict() {
    ...
    if (!this.conflicts.length) {
      import_coc11.window.showMessage("No conflicts detected", "warning");
      return;
    }
    let line = await nvim.call("line", ".");
  ...
  async conflictKeepPart(part) {
    ...
    if (!conflicts || conflicts.length == 0) {
      import_coc11.window.showMessage("No conflicts detected");
      return;
    }
    let line = await nvim.call("line", ".");
  ...
  async browser(action = "open", range) {
    ...
    let {nvim} = import_coc11.workspace;
    if (!output.trim()) {
      import_coc11.window.showMessage(`No remote found`, "warning");
      return;
    }
    let head = await this.repo.safeRun(["symbolic-ref", "--short", "-q", "HEAD"]);
    head = head.trim();
    if (!head.length) {
      import_coc11.window.showMessage(`Failed on git symbolic-ref`, "warning");
      return;
    }
  ...
  async showCommit() {
    let indexed = await this.repo.isIndexed(this.relpath);
    if (!indexed) {
      import_coc11.window.showMessage(`"${this.relpath}" not indexed.`, "warning");
      return;
    }
    ...
    if (/^0+$/.test(commit)) {
      import_coc11.window.showMessage("not committed yet!", "warning");
      return;
    }
  ...
  async toggleFold() {
    ...
    if (!infos || infos.length == 0) {
      import_coc11.window.showMessage("No changes", "warning");
      return;
    }
  ...
  async showDoc(content, filetype = "diff") {
    if (import_coc11.workspace.floatSupported) {
      let docs = [{content, filetype}];
      await this.floatFactory.show(docs);
    } else {
      const lines = content.split("\n");
      import_coc11.workspace.nvim.call("coc#util#preview_info", [lines, "diff"], true);
    }
  }
...
var GitService = class {
  constructor(gitInfo) {
    this.repos = new Map();
    const outputChannel = this.outputChannel = import_coc12.window.createOutputChannel("git");
    this._git = new git_default(gitInfo, outputChannel);
    this._resolver = new resolver_default(this._git, outputChannel);
    this.floatFactory = new import_coc12.FloatFactory(import_coc12.workspace.nvim);
  }
  ...
  async getCurrentRepo() {
    ...
    if (!import_path7.default.isAbsolute(fullpath) || buftype != "")
      return void 0;
    let root = await this.resolver.resolveGitRoot({uri: import_coc12.Uri.file(fullpath).toString(), schema: "file", buftype: ""});
    if (root == null)
      return void 0;
...
function issuesFiletypes() {
  return import_coc13.workspace.getConfiguration().get("coc.source.issues.filetypes");
}
...
function addSource(context, resolver) {
  ...
  let statusItem = import_coc13.window.createStatusBarItem(0, {progress: true});
  ...
  async function loadGitLabIssues(res, host) {
    ...
    while (true) {
      ...
      pageIndex++;
      try {
        let info = await import_coc13.fetch(pageUri, {headers: {"Private-Token": token}});
        if (!info.length) {
          break;
        }
  ...
  for (let doc of import_coc13.workspace.documents) {
    if (issuesFiletypes().includes(doc.filetype)) {
      loadIssuesFromDocument(doc);
    }
  }
  import_coc13.workspace.onDidOpenTextDocument(async (e) => {
    if (issuesFiletypes().includes(e.languageId)) {
      let doc = import_coc13.workspace.getDocument(e.uri);
      loadIssuesFromDocument(doc);
    }
  }, null, subscriptions);
  let source = {
    ...
    async doComplete(opt) {
      const config = import_coc13.workspace.getConfiguration("git");
  ...
  subscriptions.push(import_coc13.sources.createSource(source));
  let list = {
    name: "issues",
    actions: [{
      name: "open",
    ...
    loadItems: async (context2) => {
      ...
      let root = await resolver.resolveGitRoot(import_coc13.workspace.getDocument(buf.id));
      if (!root)
        return [];
      let issues = await loadIssues(root);
      return issues.map((o) => {
        return {
          label: `${import_safe3.default.red("#" + o.id.toFixed(0))} ${o.title} (${import_safe3.default.green(o.createAt.toUTCString())}) <${import_safe3.default.blue(o.creator)}>`,
          data: {id: o.id, url: o.url, body: o.body}
        };
      });
    }
  };
  subscriptions.push(import_coc13.listManager.registerList(list));
}
...
async function activate(context) {
  ...
  try {
    ...
    import_coc14.window.showMessage("git command required for coc-git", "error");
    return;
  }
  const virtualTextSrcId = await import_coc14.workspace.nvim.createNamespace("coc-git-virtual");
  const conflictSrcId = await import_coc14.workspace.nvim.createNamespace("coc-git-conflicts");
  ...
  subscriptions.push(import_coc14.commands.registerCommand("git.refresh", () => {
    manager.refresh();
  }));
  subscriptions.push(import_coc14.workspace.registerKeymap(["n"], "git-nextchunk", async () => {
    await manager.nextChunk();
  }, {sync: false}));
  subscriptions.push(import_coc14.workspace.registerKeymap(["n"], "git-prevchunk", async () => {
    await manager.prevChunk();
  }, {sync: false}));
  subscriptions.push(import_coc14.workspace.registerKeymap(["n"], "git-nextconflict", async () => {
    await manager.nextConflict();
  }, {sync: false}));
  subscriptions.push(import_coc14.workspace.registerKeymap(["n"], "git-prevconflict", async () => {
    await manager.prevConflict();
  }, {sync: false}));
  subscriptions.push(import_coc14.workspace.registerKeymap(["n"], "git-keepcurrent", async () => {
    await manager.keepCurrent();
  }, {sync: false}));
  subscriptions.push(import_coc14.workspace.registerKeymap(["n"], "git-keepincoming", async () => {
    await manager.keepIncoming();
  }, {sync: false}));
  subscriptions.push(import_coc14.workspace.registerKeymap(["n"], "git-chunkinfo", async () => {
  ...
  subscriptions.push(import_coc14.commands.registerCommand("git.chunkInfo", async () => {
    await manager.chunkInfo();
  }));
  ...
  subscriptions.push(import_coc14.commands.registerCommand("git.showCommit", async () => {
    await manager.showCommit();
  }));
  subscriptions.push(import_coc14.commands.registerCommand("git.browserOpen", async () => {
  ...
  subscriptions.push(import_coc14.commands.registerCommand("git.diffCached", async () => {
    await manager.diffCached();
  }));
  subscriptions.push(import_coc14.commands.registerCommand("git.toggleGutters", async () => {
  ...
  subscriptions.push(import_coc14.listManager.registerList(new bcommits_default(nvim, manager)));
  subscriptions.push(import_coc14.listManager.registerList(new gfiles_default(nvim, manager)));
  subscriptions.push(import_coc14.languages.registerCompletionItemProvider("semantic-commit", "Commit", config.get("semanticCommit.filetypes"), {
    provideCompletionItems: async (document, position) => {
      if (position.line !== 0) {
        return [];
      }
      const text = document.getText(import_coc14.Range.create(import_coc14.Position.create(position.line, 0), position));
      if (/^[a-z]*$/.test(text)) {
  ...
  subscriptions.push(import_coc14.workspace.registerKeymap(["o", "x"], "git-chunk-inner", async () => {
    let diff = await manager.getCurrentChunk();
    if (!diff)
      return;
    await nvim.command(`normal! ${diff.start}GV${diff.end}G`);
  }, {sync: true, silent: true}));
  subscriptions.push(import_coc14.workspace.registerKeymap(["o", "x"], "git-chunk-outer", async () => {
    let diff = await manager.getCurrentChunk();
    if (!diff)
      return;
