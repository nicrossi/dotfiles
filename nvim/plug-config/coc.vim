...
function! s:checkVersion() abort
  ...
  if get(g:, 'coc_disable_startup_warning', 0) != 1
    ...
    if l:unsupported == 1
      ...
      echom "Please do not report bugs unless you're using a least Vim 8.0.1453 or Neovim 0.3.2."
      echohl None
      sleep 2
    else
...
onoremap <silent> <Plug>(coc-classobj-a)       :<C-U>call coc#rpc#request('selectSymbolRange', [v:false, '', ['Interface', 'Struct', 'Class']])<CR>t
